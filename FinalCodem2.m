...
hidden_image=imresize(hidden_image,[180 180]);
pattern=[1 1];
...
% Now embedding the encoded code using LT
...
for i=1:rows_covered                    % 1 2 3 .... rows_covered
    for j=1:cols_covered                % 1 2 3 .... cols_covered
        ...
        transformed_block=uint32(Lag_Transform2(uint32(single_block))); 
        ...
        for current_pixel_row=1:pixel_group_rows    % 1
         ...
         In_one_block=uint32(Inv_Lag_Transform2(int32(transformed_block)));
...
for i=1:extraction_block_rows      
    for j=1:extraction_block_cols  
        ...
        % Applying Laguerre Transform on the selected block
        ex_tn_one_block=uint32(Lag_Transform2(uint32(ex_one_block)));
